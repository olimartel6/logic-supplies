...
export async function importFutechCatalog(
  ...
  try {
    ...
    if (page.url().toLowerCase().includes('/login') || !page.url().includes('futech.ca')) {
      return { total: 0, error: 'Login Futech échoué' };
    }
        ...
        try {
          ...
          products = await page.evaluate(() => {
            ...
            const cards = document.querySelectorAll(
              '.product-item, .product-card, [class*="product-item"], li[class*="item"]'
            );
