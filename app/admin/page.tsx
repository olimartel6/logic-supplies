    ...
    <div className="pb-20 md:pb-6 md:ml-56">
      ...
      <div className="max-w-lg mx-auto px-4 py-6 md:max-w-none md:mx-0">
        ...
        <div className={`${tab !== 'users' ? 'hidden' : ''} md:block`}>
          <h2 className="hidden md:block text-lg font-semibold text-gray-700 mb-3">Utilisateurs</h2>
...
<select value={userForm.role} onChange={e => setUserForm({ ...userForm, role: e.target.value })} className="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        ...
        <div className={`${tab !== 'sites' ? 'hidden' : ''} md:block`}>
          <h2 className="hidden md:block text-lg font-semibold text-gray-700 mb-3">Chantiers</h2>
            <div className="flex items-center justify-between mb-3">
