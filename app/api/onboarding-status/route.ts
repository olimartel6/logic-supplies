...
export async function GET() {
  ...
  const hasEmployee = !!(db.prepare("SELECT 1 FROM users WHERE company_id = ? AND role = 'electrician' LIMIT 1").get(ctx.companyId));
  ...
  return NextResponse.json({
    ...
    steps: [
      ...
      { key: 'employee', label: 'Ajouter un électricien', done: hasEmployee, link: '/admin' },
