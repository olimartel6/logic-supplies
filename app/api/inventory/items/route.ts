...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = db.prepare(
      'INSERT INTO inventory_items (company_id, barcode, name, unit, description, min_stock) VALUES (?, ?, ?, ?, ?, ?)'
    ).run(ctx.companyId, barcode, name, unit || 'unité', description || null, min_stock ?? null);
